...
@onready var timestamp_template: PackedScene = load("res://features/editor/editor_ui_list_item.tscn")
...
func setup_ui(title: String, audio: String, difficulty: String, timestamps: Array) -> void:
	...
	update_timestamp_list(timestamps)

func update_timestamp_list(timestamps):
	for child in timestamp_list.get_children():
		timestamp_list.remove_child(child)
